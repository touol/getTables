...
class getTable
{
	...
	public function generateData($table,$pdoConfig =[])
	{
		...
		foreach($rows as $k => $row){
			...
			foreach($r['tds'] as $ktd=>&$td){
				...
				//$this->pdoTools->addTime("getTable generateData td ".print_r($td,1));
				if(($td['edit']['default'] or $td['edit']['force']) and empty($td['value'])){
					if(!$td['edit']['force'] and $td['edit']['default']) $td['edit']['force'] = $td['edit']['default'];
					switch($td['edit']['type']){
						case 'date':
							$td['edit']['force'] = date('Y-m-d',strtotime($td['edit']['force']));
							break;
					}
					switch($td['edit']['force']){
						case 'user_id':
							$td['edit']['force'] = $this->modx->user->id;
							break;
					}
	...
	public function addFilterTable($table)
	{
		...
		foreach($table['filters'] as $k=>&$filter){
			...
			}else if(!empty($filter['default'])){
				//$this->pdoTools->addTime("getTable filter default ".print_r($filter['default'],1));
				if(in_array($this->modx->user->id,$filter['default']['modx_user_id'])){
					$query[$filter['where_field']] = $this->modx->user->id;
				}
				if(!empty($filter['default']['from'])){
					$date['from'] = date('Y-m-d',strtotime($filter['default']['from']));
				}
				if(is_array($filter['default']['in'])){
					$query[$filter['edit']['where_field']] = $filter['default']['in'];
					$filter['value'] = $filter['default']['in'];
					//$date['from'] = date('Y-m-d',strtotime($filter['default']['from']));
				}
			}else{
				$filter['value'] = '';
			...
			//'<input type="text" name="'.$filter['edit']['field'].'" value="'.$filter['value'].'" class="form-control" placeholder="'.$filter['edit']['field'].'">';
		...
		foreach($table['filters'] as $f){
			$table['topBar'][$f['section']]['filters'][] = $f;
		}
	...
	public function getStyleChunks()
	{
		if($this->config['frontend_framework_style'] != 'bootstrap_v3'){
			...
			'getTableFilterTpl' => 'getTable.Filter.tpl',
				*/
				if($this->config['getTableNavTpl'] == 'getTable.nav.tpl'){
					if($chunk = $this->modx->getObject('modChunk', array('name' => $propSet->getTableNavTpl))){
						$this->config['getTableNavTpl'] = $propSet->getTableNavTpl;
					}
				}
				if($this->config['getTableEditRowTpl'] == 'getTable.EditRow.tpl'){
					if($chunk = $this->modx->getObject('modChunk', array('name' => $propSet->getTableEditRowTpl))){
						$this->config['getTableEditRowTpl'] = $propSet->getTableEditRowTpl;
					}
				}
				if($this->config['getTableFilterTpl'] == 'getTable.Filter.tpl'){
					if($chunk = $this->modx->getObject('modChunk', array('name' => $propSet->getTableFilterTpl))){
						$this->config['getTableFilterTpl'] = $propSet->getTableFilterTpl;
					}
				}
			}
		}
